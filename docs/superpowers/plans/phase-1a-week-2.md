...
**Day 10 목표 (spec §6-1, §6-2 정정, §5-8):** 모든 산출물을 정리해 **Phase 1A 단독 6~7페이지 포트폴리오** 1차 제출본을 작성 (Airflow 본진 4 DAG 페이지 포함). spec §6-3 의 7개 페이지 구조를 따른다. 추가로 **(α) SLO 정의 두 가지 분리 정정** (data freshness + platform latency, spec §6-2 단일 출처) + **(β) `slo_daily_report` DAG** (BranchPythonOperator) 를 작성해 spec §5-8 표 4행을 채우고 본진 4 DAG 라인업을 완성. **체크포인트 1 = 본 plan 의 종료점**.

**Task 작업 순서 정당화:** 10.0 SLO 정의 정정 → 10.3 `slo_daily_report` DAG (두 가지 SLO 측정) → 10.1 다이어그램 → 10.2 포트폴리오 → 10.4 README + 최종 게이트. **순서 사유**: 10.0+10.3 (PR α) 의 SLO 분리 결정이 10.2 포트폴리오 p4 (SLO 페이지) 본문 + 10.4 README 의 SLO 라인 수치에 직접 영향 → 코드/DAG 완성 후 docs 작성이 drift 없음. 10.3 의 manual trigger 1회로 두 가지 SLO 의 첫 측정값 산출 → 포트폴리오에 그대로 인용.
...
### Task 10.0: SLO 정의 두 가지 분리 정정 (PR α)
...
- Modify: `src/flink_jobs/slo_metrics.py` (두 가지 SLO 측정 함수 + `SLOReport` dataclass)
...
- Modify: `tests/unit/flink_jobs/test_slo_metrics.py` (회귀 0 + 두 가지 SLO 측정 단위 테스트 추가)
...
- `uv run --extra flink python -m flink_jobs.slo_metrics` → 두 가지 SLO 모두 보고
...
### Task 10.3: `slo_daily_report` DAG — BranchPythonOperator + 두 가지 SLO (PR α, Airflow 본진 4 DAG 라인업 완성)

**Files:**
- Create: `airflow/dags/slo_daily_report.py`
- Create: `airflow/dags/common/slo_query.py` — DuckDB 로 Iceberg `gold.fact_hotspot_congestion_5min` 의 두 가지 SLO 메트릭 집계 (data freshness + platform latency, percentile_cont)
...
- Modify: `airflow/dags/common/callbacks.py` — `send_slo_alert(report)` Discord webhook helper. report = 두 가지 메트릭 포함
- (Phase 2 대비) Iceberg 테이블 `archive.fact_slo_daily` DDL 메모만 작성, 본격 적재는 Phase 2

**Goal:** `airflow dags trigger slo_daily_report` 으로 어제 하루의 두 가지 SLO P95 측정 → `data_freshness_p95 > 45m OR platform_latency_p95 > 7m` 이면 `send_slo_alert` 분기 (어느 SLO 가 위반인지 메시지에 명시), 둘 다 SLO 안 위반이면 `skip_alert`. 정상 일자엔 noise 0. 다음 날 09:00 KST 자동 실행 확인.

**본진 기능 발휘 (spec §5-8 표 4행):**
- **BranchPythonOperator**: `branch_on_slo_violation` 이 XCom 의 두 가지 P95 를 읽어 `"send_alert"` 또는 `"skip_alert"` task_id 반환 → Airflow 가 자동으로 한쪽만 실행
- **XCom 흐름**: `collect_slo_metrics` (push: `{data_freshness: {p50, p95, p99, count}, platform_latency: {p50, p95, p99, count}}`) → `generate_report` (pull → Jinja template → markdown) → `branch_on_slo_violation` (pull → 둘 중 하나라도 위반이면 분기) → `send_alert` (pull → Discord 메시지에 두 가지 메트릭 + 위반 SLO 명시)
...
- Step 1: 실패 테스트 작성
  ...
  - `test_send_alert_payload_includes_both_metrics()` — Discord webhook payload 에 두 가지 메트릭 모두 포함 + 위반 SLO 명시
...
- Step 5: Airflow UI 에서 DAG 보임 + manual trigger 1회 실행 (실 데이터 두 가지 P95 산출). data freshness 또는 platform latency 위반이면 `send_alert` 분기, 둘 다 안 위반이면 `skip_alert` 분기 동작 시각 확인 (Graph view 색상)
...
- Airflow UI > Graph view 에서 한쪽 분기만 색칠된 것 확인 (두 가지 SLO 측정값에 따라 send_alert 또는 skip_alert)
...
- ✅ **Day 10 = 본 task — `slo_daily_report` (BranchPythonOperator + 두 가지 SLO + XCom + on_failure_callback)**
...
- **두 가지 SLO**: Data Freshness P95 < 45분 (서울 OpenAPI source lag 포함) / Platform Latency P95 < 7분 (Kafka → Iceberg, 우리 통제 구간)
- 운영 비용: **월 $0~$0.83**
